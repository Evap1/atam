...
check_nodes:
    ...
    # Check if there are at least 2 elements from the left
...
inner_left_check:
    ...
    # Compare values for monotonicity
    cmp %eax, %ebx              # Compare prev->data with prev of prev->data
    jg check_left_ecx_ebx       # If prev > prev of prev, check next
    jl not_monotonic            # If prev < prev of prev, it's not monotonic
...
check_left_ecx_ebx:
    ...
    jg not_monotonic            # If not in monotonic order, jump

    jmp inner_left_check        # Continue checking for the next node

right_check:
    # Check if there are at least 2 elements to the right
...
inner_right_check:
    ...
    jl not_monotonic            # If next < next of next, it's not monotonic
...
check_right_ecx_ebx:
    ...
    jmp inner_right_check       # Continue checking for the next node
...
not_monotonic:
    incq %r8                     # Increment index i
    jmp check_nodes              # Jump to next node
